...
自 v56 起，AI 助手支持 **「💬 正常 / 🧠 集群」双模式**：正常模式为快速本地答案；集群模式由 5 个智能体（检索官 / 推理官 / 网页研究员 / 编辑官 / 质检官）协作，可在站内题库、PubChem、维基百科、Bing 等来源间**按需联网检索**，并对网页资料与实验讲义做**权威冲突校验**。平台还内置 **4 部本地录制教学视频**（制备 / 性质 / 配离子电荷）与 **白天 / 夜晚双主题**（导航栏 🌓 一键切换，自动记忆偏好），构建"图文 + 视频 + AI 问答"三位一体的学习闭环。
...
- **集群模式**：5-agent 集群工作台 + 按置信度自适应联网检索（低置信度自动触发；含"搜索/查一下/pubchem"等词手动触发），支持 重新生成 / 加强网页检索 / LLM重答 / 集群状态 操作；
...
├── scripts/                   # 工具脚本（lib-assistant-faq.js、v44-inject-bundle.js、v45-round.js 等）
...
- **自定义域名迁移**：`fec2o4.apay.eu.cc` 因被网络按主机名拦截（同一 GitHub IP 下 `github.io` 可访问、唯该域名握手被 reset）而弃用；站点改用专属子域名 **`k3fec2o43.clawdbot.ggff.net`**（`ggff.net` 后缀实测无代理可握手，且不影响根基 `clawdbot.ggff.net` 上的 ClawDBot 服务）。生效步骤：① 在 Cloudflare 为 `ggff.net` zone 新增一条 **DNS-only（灰云）`k3fec2o43` → CNAME `littlealety.github.io`**；② 仓库 Settings→Pages 设自定义域名 `k3fec2o43.clawdbot.ggff.net` 并勾 **Enforce HTTPS**，等 GitHub 自动签 Let's Encrypt（数分钟~24h）后即可用 `https://k3fec2o43.clawdbot.ggff.net/` 无代理访问。
...
> 完整版本时间线 v30 → v56（2026-07 至 2026-08-18）。v45 起版本号由「语料自学习轮次 + UI 注入脚本」共同承载。
...
| **v41** | 08-13 | **全文献深度学习 + 权威对齐**：355 篇语料再学习补 57 条 FAQ（**833→890**）；按武汉大学讲义全库对齐（知识权威层级确立）；六集群审计修正 ~188 处 |
...
| 5 轮门禁收尾 | → 3047 | 门禁 9.5：avg 4.28→9.92，final 199/199 avg 9.97 |
