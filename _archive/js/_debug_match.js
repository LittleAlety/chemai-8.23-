...
for (const f of faq) {
  ...
  if (exactQ || (fq.length >= 15 && (nq.includes(fq) || fq.includes(nq)))) score += 50;
  scored.push({ score, exactQ, kh, eh, title: f.title.slice(0, 26), q: (f.q || '').slice(0, 20) });
}
scored.sort((a, b) => b.score - a.score);
console.log('Q039 匹配 Top5:');
scored.slice(0, 5).forEach(s => console.log('  ', s.score.toFixed(1), 'exactQ=' + s.exactQ, 'kh=' + s.kh, 'eh=' + s.eh, '|', s.title));
