...
const targets = qs.filter(q => byId[q.id].score < 9.5);
const KEY = process.env.DEEPSEEK_KEY, MODEL = process.env.DEEPSEEK_MODEL || 'deepseek-v4-flash';
const SYS = '你是 ChemAI 评分官。对每条给出"AI助手本地回复"对照"标准参考答案"的评分，满分10分。只输出 JSON 数组，每项：{"question":"原题","score":0-10小数一位,"why":"一句原因"}。评分准则：回复准确且覆盖参考答案关键点且与讲义一致→9.5以上。';
...
(async () => {
  ...
  for (const q of targets) {
    ...
    if (sc >= 9.5) pass++;
    console.log(q.id, '→', sc, sc >= 9.5 ? '✓' : '✗ ' + (out ? out[0].why.slice(0, 60) : ''));
  }
  console.log('\n达标: ' + pass + '/' + targets.length);
})();
