'use strict';
// 全量隔离重评（并发8）：核验重复7最终状态 → final_r7_rescore.json
const fs = require('fs');
...
(async () => {
  ...
  fs.writeFileSync(path.join(root, 'Agent工作区/Agent-C-答案评分/final_r7_rescore.json'), JSON.stringify(qs.map((q, i) => ({ id: q.id, score: nums[i] })), null, 2), 'utf8');
  console.log('DONE');
