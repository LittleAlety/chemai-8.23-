...
(function () {
  ...
  function getStateHTML() {
    ...
    /* ---------- 技能：🧮 计算官（确定性定量计算）——统一代理到 scripts/lib-calc.js 单一真相源 ----------
     原 ATOMIC_MASS / KNOWN_MASS / formulaMass / molarMassOf / calcSkill 在本文件内重复实现（含易错
     formulaMass 与三处摩尔质量常量之一），现统一改为引用浏览器 window.ChemCalc，与 local_answer.js
     / evaluate.js / assistant.html 一致，消除跨文件摩尔质量漂移。calcSkill 返回
     {matched,type,title,lines,formula,note}（lib-calc 额外带 result 字段，调用方不受影响）。 */
  function molarMassOf(term) {
    return (window.ChemCalc && window.ChemCalc.molarMassOf) ? window.ChemCalc.molarMassOf(term) : null;
  }
  function calcSkill(q) {
    if (window.ChemCalc && window.ChemCalc.calcAnswer) {
      try { return window.ChemCalc.calcAnswer(String(q || '')); } catch (e) {}
    }
    return { matched: false, type: '', title: '', lines: [], formula: '', note: '' };
  }
    ...
    return loadKG().then(function (nodes) {
      ...
      var top = scored.slice(0, 3).map(function (x) {
        ...
        return {
          ...
          name: n.name,
          category: n.category,
          description: String(n.description || '').slice(0, 150),
          subfield: n.subfield || '',
          url: 'knowledge.html?node=' + encodeURIComponent(n.id)
        };
      });
      return { matched: top.length > 0, items: top };
    });
