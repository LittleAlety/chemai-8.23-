...
async function fillMissingAnswers(answers) {
  const digest = manualDigest();
  const system = '你是 ChemAI 实验助手。直接给出回答正文，不要 JSON、不要 Markdown 代码块、不要引用题号。';
  for (let i = 0; i < answers.length; i++) {
    if (answers[i] && String(answers[i].answer || '').trim()) continue;
    const q = QUESTIONS[i];
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (String(out || '').trim()) {
          answers[i].answer = String(out).replace(/^```[\s\S]*?\n?/, '').replace(/\n?```$/, '').trim();
          break;
        }
      } catch (e) {
        console.log('retry missing answer', i, e.message);
      }
    }
...
async function scoreAll(questions, answers) {
  ...
  if (holes.length) {
    console.log('filling individual scores', holes.length);
    const systemSingle = '你是 ChemAI 评分官。只输出 JSON 对象，不要 Markdown：{"question":"原题","score":0-100,"accuracy":0-100,"completeness":0-100,"manualCompliance":0-100,"safety":0-100,"comment":"一句话评价"}';
    for (const idx of holes) {
      ...
      for (let attempt = 0; attempt < 3 && results[idx] == null; attempt++) {
        try {
          const out = await llm([
          ...
          if (items && items[0]) {
            results[idx] = { question: q.question, ...items[0] };
          }
        } catch (e) {
          console.log('single score retry', idx, e.message);
        }
      }
      if (results[idx] == null) {
        const out = await llm([
          { role: 'system', content: '你是 ChemAI 评分官。只输出一行：score: 数字; comment: 简短评价' },
          { role: 'user', content: '题目：' + q.question + '\n参考答案：' + q.referenceAnswer + '\nAI回答：' + answers[idx].answer }
        ]);
        const m = String(out || '').match(/score\s*[:：]\s*(\d{1,3})/i);
        if (m) results[idx] = {
          question: q.question,
          score: m[1],
          accuracy: m[1],
          completeness: m[1],
          manualCompliance: m[1],
          safety: 100,
          comment: String(out || '').replace(/\s+/g, ' ').slice(0, 180)
        };
      }
...
(async () => {
  ...
  await fillMissingAnswers(answers);
  const scores = await scoreAll(QUESTIONS, answers);
  const validScores = scores.filter(Boolean);
  const report = {
    total: QUESTIONS.length,
    scored: validScores.length,
    avgScore: avg(validScores.map(s => Number(s.score))),
    avgAccuracy: avg(validScores.map(s => Number(s.accuracy))),
    avgCompleteness: avg(validScores.map(s => Number(s.completeness))),
    avgManualCompliance: avg(validScores.map(s => Number(s.manualCompliance))),
